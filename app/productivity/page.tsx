...
// --- START: ChartJS registration (Kept outside component) ---
// Note: Assuming Chart.js related imports are commented out or handled elsewhere,
// but I'll leave the placeholders as per the previous version's scope integrity fix.
// --- END: ChartJS registration ---
...
export default function ProductivityPage() {
  ...
  const [events, setEvents] = useState<{[key: string]: string[]}>({
      // Demo Event to match screenshot's visual style
  ...
  function handleToday() {
    setCMonth(today.getMonth());
    setCYear(today.getFullYear());
  ...
  // Determine if a given date is *currently* today
  ...
  // --- Reusable Selector Component (FIXED TYPES) ---
                ...
                {items.map((item, idx) => (
                    <button
                        ...
                        className={`block w-full text-left px-4 py-2 text-sm transition-colors ${
                            idx === currentIdx ? 'bg-[#b773f8] text-black font-semibold' : 'text-white hover:bg-[#29253b]'
                        }`}
